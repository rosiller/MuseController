...
class EEG:
    ...
    def _start_muse(self, duration, ppg, acc, gyro):
        ...
        if sys.platform in ["linux", "linux2", "darwin"]:
            ...
            self.stream_process = Process(
                target=stream, args=(self.muses[0]["address"], ), kwargs={'ppg_enabled':ppg, 'acc_enabled': acc, 'gyro_enabled': gyro}
            )
            self.stream_process.start()

        # Create markers stream outlet
        self.muse_StreamInfo = StreamInfo(
            name="Markers", type ="Markers", channel_count=1, nominal_srate=0, channel_format = "int32", source_id = "myuidw43536"
        )
    ...
    def start(self, fn, duration=None, ppg=False, acc=False,gyro=False):
        ...
        if self.backend == "brainflow":  # Start brainflow backend
            ...
            self._start_muse(duration,ppg, acc, gyro)
    ...
    def get_recent(self,max_samples=100):
        """
        Usage:
        -------
        ...
        if self.backend == "brainflow":
            ...
            df = self._muse_get_recent(max_samples)
